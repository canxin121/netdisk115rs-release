...
新增 Rust 分层结构化日志，改进会话、编辑器和播放器可靠性；受管整理采用原生精确映射批量改名，移动逐项确认，并强化索引分页、变更窗口和缓存失效。旧数据库继续保留，日志文件默认关闭。完整变化与自建客户端兼容提示见 [v0.4.0 发布说明](releases/v0.4.0.md)。

## 媒体库维护范围（v0.3.0 起）

媒体中心只保留项目自己的本地索引与媒体资料库：电影、剧集、专辑、歌曲、图片、搜索、同步、备份恢复和跨账号副本管理。
115 原生观看／我听、旧音乐听单、原生相册及其 API/CLI 已移除；音视频进度只保存到本地，不再同步 115 原生播放历史。
移除的是程序支持，不会删除原有本地数据库或 115 上的文件、观看记录、听单和相册。

视频清晰度选择、外部播放票据、QuickLook、文件下载和字幕继续保留。完整的兼容性变化与升级说明见 [v0.3.0 发布说明](releases/v0.3.0.md)。
